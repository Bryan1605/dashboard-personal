...
function deleteCategory(type, name) {
  if (!confirm(`??Eliminar "${name}"? Los elementos asociados se mover??n a "General".`)) return;
  const db = getDB();
  if (db.categories[type].length <= 1) return alert('Debe haber al menos una categor??a.');
...
function renderHabitMiniList() {
  ...
  if (habits.length === 0) {
    container.innerHTML = '<p class="text-secondary text-center">No hay h??bitos activos</p>';
    return;
  }
  ...
  if (habits.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <p class="text-secondary">Crea tu primer h??bito abajo</p>
      </div>
    `;
    return;
  }
    ...
    return `
      <div class="habit-card animate-in">
        <div class="habit-header">
          ...
          <div class="habit-content" style="flex: 1;">
            ...
            <div class="habit-meta">
              ...
              ${streak > 0 ? `<span class="habit-streak">???? ${streak} d??as</span>` : ''}
            </div>
        ...
        <div class="habit-actions">
          <button class="btn btn-sm btn-ghost" onclick="openEditModal(${h.id})" title="Editar">??????</button>
          ...
          <button class="btn btn-sm btn-ghost" onclick="deleteHabit(${h.id})" title="Eliminar" style="color: var(--danger);">???????</button>
        </div>
      </div>
    `;
  }).join('');
...
function resetStreak(id) {
  if (!confirm('??Reiniciar la racha de este h??bito?')) return;
  const db = getDB();
  const habit = db.habits.find(h => h.id === id);
  if (habit) {
    habit.history = [];
...
function deleteHabit(id) {
  if (!confirm('??Eliminar este h??bito permanentemente?')) return;
  const db = getDB();
